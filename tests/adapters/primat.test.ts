...
const product = (id:string, name:string, path:string[], unit:Product["packageUnit"]="g"): Product => ({ id, name, concept:"banan", brand:null, priceOre:ore(2_000), packageSize:500, packageUnit:unit, comparison:{priceOre:ore(4_000),unit:unit === "g" ? "kg" : "st"}, categoryPath:path,dietaryTags:[] });
...
describe("adapter contracts: raw-fed mapper and fixture implementation", () => {
  it("returns identical normalized stores", async () => {
    const mapped = mapResolveResponse(raw<PrimatResolveResponse>("resolve-umea.json")); const fixture = await new FixtureStoreDiscovery().resolve(null, options);
  ...
  it("returns identical normalized products and regular quotes", async () => {
    ...
    const response = raw<PrimatProductsResponse>("products-banan.json"); const expected = response.data.filter((item) => item.available && item.chain === "coop" && item.store_id === "232400").map((item) => { try{return mapProduct(item,"banan")}catch{return null} }).filter((item):item is Product=>item!==null).slice(0,5);
    const fixture = await new FixtureProductSearch().search({concept:"banan",store:storeOption,limit:5},options); expect(fixture).toEqual(expected);
    const quotes = await new FixturePriceSource().quote(fixture.map((item)=>item.id),storeOption,options); expect(quotes.every((quote)=>quote.priceType === "regular" && Number.isInteger(quote.priceOre))).toBe(true);
  });
});
...
describe("Primat mapping rules", () => {
  ...
  it("surfaces mandatory attribution",()=>expect(PRIMAT_ATTRIBUTION).toBe("Prisdata från primat.nu"));
});
