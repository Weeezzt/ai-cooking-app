...
const DERIVATIVE_CATEGORY = /saser|sas|kryddor|snacks|chips|godis|dryck|juice/iu;

export function ingredientNameMatches(ingredientName: string, productName: string): boolean {
  const wanted = fold(ingredientName);
  const actual = fold(productName);
  const whole = new RegExp(`(?:^|[^a-z0-9])${escape(wanted)}(?:$|[^a-z0-9])`, "u");
  if (whole.test(actual)) return true;
  const compactWanted = wanted.replace(/[^a-z0-9]/g, "");
  const compactActual = actual.replace(/[^a-z0-9]/g, "");
  const index = compactActual.indexOf(compactWanted);
  if (index < 0) return false;
  const prefix = compactActual.slice(0,index);
  const suffix = compactActual.slice(index+compactWanted.length);
  return /^(|farsk|ekologisk|svensk)$/u.test(prefix) && /^(|pasta|file|fars|soppa|gryta|sas|pulver|olja|gradde|ost|more)$/u.test(suffix);
}
...
function acceptable(ingredient: ResolvableIngredient, product: Product): CandidateRejection["reason"] | null {
  ...
  if (product.section !== ingredient.kategori && ingredient.kategori !== "ÖVRIGT" && product.section !== "ÖVRIGT") return "concept_mismatch";
